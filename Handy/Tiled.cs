...
namespace Squared.Tiled
{
    public class Tileset
    {
        ...
        internal bool MapTileToRect(int index, ref Rectangle rect)
        {
            ...
            var rowSize = _TexWidth / (TileWidth + Spacing);
            var row = index / rowSize;
            var numRows = _TexHeight / (TileHeight + Spacing);
            if (row >= numRows)
                return false;
    ...
    public class Layer
    {
        ...
        internal TileInfo[] _TileInfoCache;
        ...
        internal static Layer Load(XmlReader reader)
        {
            ...
            if (reader.GetAttribute("name") != null) result.Name = reader.GetAttribute("name");
            if (reader.GetAttribute("width") != null) result.Width = int.Parse(reader.GetAttribute("width"));
            if (reader.GetAttribute("height") != null) result.Height = int.Parse(reader.GetAttribute("height"));
                            ...
                            case "data":
                            {
                                if (reader.GetAttribute("encoding") != null)
                                {
                                    ...
                                    switch (encoding)
                                    {
                                        case "base64":
                                        {
                                            var dataSize = result.Width * result.Height * 4 + 1024;
        ...
        protected void BuildTileInfoCache(IList<Tileset> tilesets)
        {
            ...
            next:
            for (var t = 0; t < tilesets.Count; t++)
        ...
        public void Draw(SpriteBatch batch, IList<Tileset> tilesets, Rectangle rectangle, Vector2 viewportPosition,
            int tileWidth, int tileHeight)
        {
            var i = 0;
            var destPos = new Vector2(rectangle.Left, rectangle.Top);
            var viewPos = viewportPosition;

            var minX = (int) Math.Floor(viewportPosition.X / tileWidth);
            var minY = (int) Math.Floor(viewportPosition.Y / tileHeight);
            var maxX = (int) Math.Ceiling((rectangle.Width + viewportPosition.X) / tileWidth);
            var maxY = (int) Math.Ceiling((rectangle.Height + viewportPosition.Y) / tileHeight);
            ...
            for (var y = minY; y <= maxY; y++)
            {
                ...
                for (var x = minX; x <= maxX; x++)
                {
                    ...
                    var flipAndRotate = FlipAndRotate[i];
                    var flipEffect = SpriteEffects.None;
                    var rotation = 0f;

                    if ((flipAndRotate & HorizontalFlipDrawFlag) != 0) flipEffect |= SpriteEffects.FlipHorizontally;
                    if ((flipAndRotate & VerticalFlipDrawFlag) != 0) flipEffect |= SpriteEffects.FlipVertically;
                        ...
                        switch (name)
                        {
                            case "object":
                            {
                                using (var st = reader.ReadSubtree())
                                {
                                    ...
                                    {
                                        var count = result.Objects.Keys.Count(item => item.Equals(objects.Name));
        ...
        public void Draw(Map result, SpriteBatch batch, Rectangle rectangle, Vector2 viewportPosition)
        {
            foreach (var objects in Objects.Values)
                if (objects.Texture != null)
                    objects.Draw(batch, rectangle, new Vector2(X * result.TileWidth, Y * result.TileHeight),
    ...
    public class Object
    {
        ...
        internal static Object Load(XmlReader reader)
        {
             ...
             */
            int width;
            if (int.TryParse(reader.GetAttribute("width"), out width)) result.Width = width;

            int height;
            if (int.TryParse(reader.GetAttribute("height"), out height)) result.Height = height;
            ...
            while (!reader.EOF)
            {
                switch (reader.NodeType)
                {
                    case XmlNodeType.Element:
                        ...
                        if (reader.Name == "image") result.Image = reader.GetAttribute("source");
        ...
        public void Draw(SpriteBatch batch, Rectangle rectangle, Vector2 offset, Vector2 viewportPosition,
            float opacity)
        {
            var viewPos = viewportPosition;

            var minX = (int) Math.Floor(viewportPosition.X);
            var minY = (int) Math.Floor(viewportPosition.Y);
            var maxX = (int) Math.Ceiling(rectangle.Width + viewportPosition.X);
            var maxY = (int) Math.Ceiling(rectangle.Height + viewportPosition.Y);

            if (X + offset.X + Width > minX && X + offset.X < maxX)
                if (Y + offset.Y + Height > minY && Y + offset.Y < maxY)
                {
                    var x = (int) (X + offset.X - viewportPosition.X);
                    var y = (int) (Y + offset.Y - viewportPosition.Y);
                    batch.Draw(_Texture, new Rectangle(x, y, Width, Height),
                        new Rectangle(0, 0, _Texture.Width, _Texture.Height), Color.White * opacity);
                }
        }
    }
    ...
    public class Map
    {
                            ...
                            switch (name)
                            {
                                ...
                                case "layer":
                                {
                                    using (var st = reader.ReadSubtree())
                                    {
                                        ...
                                        if (null != layer) result.Layers.Add(layer.Name, layer);
                                    }
                                }
                                    break;
                                case "objectgroup":
        ...
        {
            foreach (var layers in Layers.Values)
