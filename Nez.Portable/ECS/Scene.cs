...
namespace Nez
{
    public class Scene
    {
        ...
        internal void postRender(RenderTarget2D finalRenderTarget = null)
        {
            ...
            if (enablePostProcessing)
                for (var i = 0; i < _postProcessors.length; i++)
                    if (_postProcessors.buffer[i].enabled)
                    {
                        ...
                        enabledCounter++;
                        _postProcessors.buffer[i].process(isEven ? sceneRenderTarget : _destinationRenderTarget,
            ...
            {
                if (i == 0)
                    Core.graphicsDevice.setRenderTarget(Mathf.isEven(enabledCounter)
                        ? sceneRenderTarget
        ...
        {
            Assert.isTrue(_sceneComponents.contains(component),
