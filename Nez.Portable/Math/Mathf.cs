...
namespace Nez
{
    public static class Mathf
    {
        ...
        {
            var deltaAngleRadians = Mathf.deltaAngleRadians(start, end);
        ...
        {
            var x = sin(Time.time * xFrequency + phase) * xMagnitude;
            var y = cos(Time.time * yFrequency) * yMagnitude;
        ...
        {
            var wrappedTime = pingPong(Time.time, oscillationInterval);
            var damped = pow(MathHelper.E, -damping * wrappedTime);

            var x = damped * sin(Time.time * xFrequency + phase) * xMagnitude;
            var y = damped * cos(Time.time * yFrequency) * yMagnitude;
