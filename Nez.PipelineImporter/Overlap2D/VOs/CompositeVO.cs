    ...
    public class CompositeVO
    {
        ...
        public void setLayerDepthRecursively(float zIndexMin, float zIndexMax, CompositeItemVO parentCompositeVO)
        {
            for (var i = 0; i < sImages.Count; i++)
            {
                ...
                Overlap2DImporter.logger.LogMessage("[image] zIndex: {0} -> {1}, parent: {2}, renderLayer: {3}",
                    sImages[i].zIndex, sImages[i].layerDepth, parentCompositeVO, sImages[i].renderLayer);
            }
        ...
        public void setLayerDepthRecursively(Dictionary<int, int> minIndicies, Dictionary<int, int> maxIndicies,
            CompositeItemVO parentCompositeVO)
        {
            for (var i = 0; i < sImages.Count; i++)
            {
                var renderLayer = sImages[i].renderLayer;
