...
namespace Nez.TiledMaps
{
    ...
    public class TiledMapProcessor : ContentProcessor<TmxMap, TmxMap>
    {
        ...
        private static void setTilesetTextureIfNecessary(TmxTileset tileset, ContentProcessorContext context)
        {
            ...
            var bm = new Bitmap(packedSprites.Width, packedSprites.Height);
            for (var x = 0; x < packedSprites.Width; x++)
            for (var y = 0; y < packedSprites.Height; y++)
            {
                var col = packedSprites.GetPixel(x, y);
                var color = Color.FromArgb(col.A, col.R, col.G, col.B);
                bm.SetPixel(x, y, color);
            }

            var atlasFilename = tileset.name + "-atlas.png";
            bm.Save(Path.Combine(tileset.mapFolder, atlasFilename), ImageFormat.Png);
