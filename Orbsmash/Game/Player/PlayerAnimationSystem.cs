...
namespace Orbsmash.Player
{
    public class PlayerAnimationSystem : EntitySystem
    {
        ...
        public PlayerAnimationSystem() : base(new Matcher().all(typeof(PlayerInputComponent),
        ...
        protected override void process(List<Entity> entities)
        {
            foreach (var entity in entities)
            {
                ...
                var mainBodyAnimation = playerAnimComponents
                    .Where(a => a.Context == AnimationContexts.PLAYER_SPRITE_ANIMATIONS).First();

                switch (state.StateEnum)
                {
                    case PlayerStates.Idle:
                        if (state.LastDirection == Gameplay.Direction.UP ||
                    ...
                    case PlayerStates.Charge:
                        ...
                        if (state.ChargeFinished) mainBodyAnimation.SetAnimation(PlayerAnimations.CHARGE_PULSE);
                        break;
